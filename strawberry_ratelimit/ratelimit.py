...
class ExtensionRatelimit(Extension):
    ...
    def on_executing_start(self):
        ...
        client_key = execution_context.context["request"].META["REMOTE_ADDR"]
        ...
        for func_name in self.type_name:
            ...
            if func_log:
                ...
                if rate_log[0] + self.rate_seconds >= time.time():
                    remaining_time = int(
                        self.rate_seconds - (time.time() - rate_log[0])
                    )
                    error_message = (
                        f"Ratelimit: `{func_name}`. Try again after {remaining_time}s"
                    )
                    self.execution_context.result = GraphQLExecutionResult(
                        data=None,
                        errors=[GraphQLError(error_message)],
